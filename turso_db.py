...
class TursoDatabase:
    ...
    _RECONNECT_COOLDOWN = 30  # seconds between reconnect attempts after failure
    ...
    def _execute(self, sql: str, args: Optional[List] = None) -> Optional[List[List]]:
        ...
        try:
            ...
            if resp.status_code != 200:
                print(f"Turso HTTP error {resp.status_code}: {resp.text[:200]}")
                return None

            data = resp.json()
            results = data.get('results', [])
            if not results or results[0].get('type') != 'ok':
                print(f"Turso result error: {results[0] if results else 'empty response'}")
                return None
    ...
    def load_verb_roots(self) -> set:
        ...
        if not self.connected:
            if not self._throttled_connect():
                return set()
    ...
    def _should_attempt_reconnect(self) -> bool:
        """Return True if enough time has passed since the last failed connect attempt."""
        import time
        return (time.time() - self._last_connect_attempt) >= self._RECONNECT_COOLDOWN

    def _throttled_connect(self) -> bool:
        """Connect only if not in cooldown period; update attempt timestamp."""
        import time
