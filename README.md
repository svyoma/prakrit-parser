...
| **Participles** | Absolutive (*sambandhaka-kṛdanta*), infinitive (*kriyārthaka-kṛdanta*, `-tuM` family), hetvartha (*hetvartha-kṛdanta*, `-iuM`/`-euM`/`-uM` — Jain Prakrit-native purpose form), present participle (*vartamāna-kṛdanta*), past passive participle (*bhūta-kṛdanta*), future passive participle / gerundive (*kṛtya-kṛdanta*) |
| **Declined participles** | Nominal declension of present participles (e.g., *gacchantāṇa* = gen./dat. pl. of the present participle of *gacch*) — all 8 cases fully resolved |
| **Transliteration** | Input in Devanagari is automatically converted to Harvard-Kyoto (HK) for processing |

### Phonological rules

- **Anusvara assimilation** — M/ṃ before consonants is expanded to all phonologically conditioned nasal variants for database lookup
- **Vowel sandhi reversal** — recovers vowel-final roots from sandhi-affected bases (e → ī, o → ū, a → ā)
- **Hiatus *y*-resolution** — Prakrit inserts *y* between adjacent vowels (e.g., root *nī* + suffix *-anta* → *nīyanta*); the parser identifies and strips this glide when recovering roots and stems
- **Oblique stem alternation** — the *a* of present-participle stems alternates to *e* before certain case endings in some dialects (e.g., *gacchantesu* → *gacchanta*); this is transparently reversed during analysis
...
| `/api/status` | GET | Parser and database status (Turso connectivity, roots loaded, dictionary availability) |
